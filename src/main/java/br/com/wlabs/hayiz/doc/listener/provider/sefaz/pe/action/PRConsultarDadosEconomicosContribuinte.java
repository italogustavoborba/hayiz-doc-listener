...
public class PRConsultarDadosEconomicosContribuinte extends SefazPernambucoProvider implements SQSAction {

    private Logger log = LoggerFactory.getLogger(PRConsultarDadosEconomicosContribuinte.class);
            ...
            for(Map<String, Object> document: documents) {
                ...
                try {
                    ...
                    String key = data.get("workspaceCode").toString() + "/document/" +
                            ...
                            LocalDate.now().format(DateTimeFormatter.ofPattern("yyMM")) + "/perfil-do-contribuinte-credenciamentos-" +
                            UUID.randomUUID() + ".pdf";

                    Request request = new Request.Builder()
                            .url("https://efiscoi.sefaz.pe.gov.br/sfi_trb_gcc/PRConsultarDadosEconomicosContribuinte")
                    ...
                    formData.put("CdTipoDocumentoIdentificacao", "1");
                    ...
                    request = new Request.Builder()
                            .url("https://efiscoi.sefaz.pe.gov.br/sfi_trb_gcc/PRConsultarPerfilContribuinte")
                            .method("POST", HTMLUtil.mapToFormBody(formData).build())
                            .addHeader("Content-Type", "application/x-www-form-urlencoded")
                            .build();
                    response = client.newCall(request).execute();
                    ...
                    formData = HTMLUtil.formElementToMap((FormElement) doc.selectFirst("form[name=frm_principal]"));
                    formData.put("evento", "detalharCredenciamentos");
                    formData.put("parametro_metodo_botao", "in_content_type_app_octetstream");

                    request = new Request.Builder()
                            .url("https://efiscoi.sefaz.pe.gov.br/sfi_trb_gcc/PRConsultarExtratoCadastroContribuinte")
                            .method("POST", HTMLUtil.mapToFormBody(formData).build())
                            .addHeader("Content-Type", "application/x-www-form-urlencoded")
                            .build();
                    response = client.newCall(request).execute();
                    ...
                    Element tipoMensagemUsuario = doc.selectFirst("input[name=cd_tipo_tela_mensagem_usuario]");
                    if(Objects.nonNull(tipoMensagemUsuario) &&
                            Objects.equals("3", tipoMensagemUsuario.attr("value"))) {
                        fixCSS(doc);

                        InputStream inputStream = IOUtils.toInputStream(doc.html(), mediaType.charset());
                        InputStream inputStreamPDF = PdfService.htmlToPdf(inputStream);
                        byte[] byes = PdfService.signature(inputStreamPDF, keyEntry);
                        StorageUtil.upload(key, byes, "application/pdf");

                        SQSUtil.status(document.get("id").toString(), key, "FAIL", "Acesso não autorizado!", message.getId());
                        continue;
                    }

                    boolean isInactive = false;
                    for (Element table : doc.select("table[id=table_conteudodados]")) {
                        for(Element table2: table.select("table[width=\"100%\"]")) {
                            for (Element row : table2.select("tr")) {
                                for (Element td : row.select("td")) {
                                    Element input = td.selectFirst("input[name=credenciamento]");
                                    if(input != null && input.hasAttr("value")) {
                                        if(isInactive) continue;
                                        String situacao = input.attr("value");
                                        isInactive = situacao.equalsIgnoreCase("inativo");
                                    }
                                }
                            }
                        }
                    ...
                    SQSUtil.status(document.get("id").toString(), key, (isInactive ? "FOUND_ISSUE" : "DONE"),null, message.getId());
