package br.com.wlabs.hayiz.doc.listener.provider.city.pe.recife.action;

import br.com.wlabs.hayiz.doc.listener.exception.CaptchaException;
import br.com.wlabs.hayiz.doc.listener.exception.CertificateException;
import br.com.wlabs.hayiz.doc.listener.exception.ExpiredException;
import br.com.wlabs.hayiz.doc.listener.listener.SQSAction;
import br.com.wlabs.hayiz.doc.listener.model.Message;
import br.com.wlabs.hayiz.doc.listener.provider.city.pe.recife.RecifePernambucoProvider;
import br.com.wlabs.hayiz.doc.listener.service.PdfService;
import br.com.wlabs.hayiz.doc.listener.util.HTMLUtil;
import br.com.wlabs.hayiz.doc.listener.util.HTTPUtil;
import br.com.wlabs.hayiz.doc.listener.util.SQSUtil;
import br.com.wlabs.hayiz.doc.listener.util.StorageUtil;
import com.google.gson.Gson;
import com.google.gson.GsonBuilder;
import com.google.gson.ToNumberPolicy;
import com.twocaptcha.captcha.Normal;
import okhttp3.*;
import org.apache.commons.codec.binary.Base64;
import org.apache.commons.io.IOUtils;
import org.jsoup.Jsoup;
import org.jsoup.nodes.Document;
import org.jsoup.nodes.Element;
import org.jsoup.nodes.FormElement;
import org.jsoup.select.Elements;
import org.slf4j.Logger;
import org.slf4j.LoggerFactory;
import org.springframework.cloud.aws.messaging.listener.Acknowledgment;
import org.springframework.util.StopWatch;

import java.io.InputStream;
import java.nio.charset.Charset;
import java.security.KeyStore;
import java.time.LocalDate;
import java.time.LocalDateTime;
import java.time.ZoneId;
import java.time.format.DateTimeFormatter;
import java.util.*;
import java.util.stream.Collectors;

public class ExtratoDebitoRecife extends RecifePernambucoProvider implements SQSAction {

    private Logger log = LoggerFactory.getLogger(ExtratoDebitoRecife.class);

    @Override
    public void process(Message message, String queueUrl, final String receiptHandle, String messageGroupId,
                        Acknowledgment acknowledgment) throws Exception {
        try {

            Map<String, Object> data = message.getData();

            if(data.isEmpty()) {
                acknowledgment.acknowledge();
                log.debug("Data is empty: " + message);
                return;
            }

            if(!data.containsKey("documents")) {
                acknowledgment.acknowledge();
                log.debug("Documents is empty: " + message);
                return;
            }

            ZoneId zoneId = ZoneId.of("America/Sao_Paulo");
            LocalDate localDate = LocalDate.now(zoneId);
            if(Objects.nonNull(message.getDate()) && !message.getDate().isEmpty()) {
                if(LocalDate.parse(message.getDate()).isBefore(localDate)) {
                    List<Map<String, Object>> documents = (List<Map<String, Object>>) data.get("documents");
                    for(Map<String, Object> document: documents) {
                        SQSUtil.status(document.get("id").toString(), null, "FAIL",
                                "Não foi possível obter os dados: Sistema indisponível.",
                                message.getId());
                    }
                    acknowledgment.acknowledge();
                    return;
                }
            }

            KeyStore.PrivateKeyEntry keyEntry = this.buildKeyEntry((String) data.get("certificateKey"),
                    (String) data.get("certificatePassword"));

            int visibilityTimeout = 300;
            SQSUtil.changeMessageVisibility(queueUrl, receiptHandle, visibilityTimeout);

            List<Map<String, Object>> errors = new ArrayList<>();
            List<Map<String, Object>> documents = (List<Map<String, Object>>) data.get("documents");
            for(Map<String, Object> document: documents) {
                StopWatch stopWatch = new StopWatch();
                try {
                    stopWatch.start();

                    Set<Cookie> allCookies = Collections.synchronizedSet(new HashSet<>());
                    OkHttpClient client = buildClient(allCookies);

                    String key = data.get("workspaceCode").toString() + "/document/" +
                            document.get("companyCode").toString().replaceAll("[^0-9]", "") + "/" +
                            document.get("municipalCode").toString().replaceAll("[^0-9]", "") + "/" +
                            LocalDate.now().format(DateTimeFormatter.ofPattern("yyMM")) + "/extrato-debito-" +
                            UUID.randomUUID() + ".pdf";

                    Request request = new Request.Builder()
                            .url("https://recifeemdia.recife.pe.gov.br/extratoDebitos/2")
                            .method("GET", null)
                            .build();
                    Response response = client.newCall(request).execute();
                    MediaType mediaType = response.body().contentType();
                    Document doc = Jsoup.parse(response.body().byteStream(), mediaType.charset().name(),
                            "https://recifeemdia.recife.pe.gov.br/");

                    Element lblMensagem = doc.selectFirst("label[id=lblMensagem]");
                    if(Objects.nonNull(lblMensagem) && lblMensagem.hasText()) {
                        throw new Exception(lblMensagem.text());
                    }

                    FormElement loginForm = (FormElement) doc.selectFirst("form[id=login-form]");
                    String action = loginForm.attr("action");

                    Map<String, String> formData = HTMLUtil.formElementToMap((FormElement) doc.selectFirst("form[id=login-form]"));
                    formData.put("inscricao", document.get("municipalCode").toString().replaceAll("[^0-9]", ""));

                    Element captcha = doc.selectFirst("img[id=captcha_image]");
                    if(Objects.isNull(captcha)) {
                        throw new Exception("Tente novamente.");
                    }

                    request = new Request.Builder()
                            .url("https://recifeemdia.recife.pe.gov.br/" + captcha.attr("src"))
                            .method("GET", null)
                            .build();
                    response = client.newCall(request).execute();

                    String base64 = Base64.encodeBase64String(response.body().bytes());

                    Normal normal = new Normal();
                    normal.setBase64(base64);
                    normal.setMinLen(5);
                    normal.setMaxLen(5);
                    normal.setLang("pt");

                    this.solveCaptcha(normal);

                    formData.put("captcha_code", normal.getCode());

                    request = new Request.Builder()
                            .url("https://recifeemdia.recife.pe.gov.br" + action)
                            .method("POST", HTMLUtil.mapToFormBody(formData).build())
                            .addHeader("Content-Type", "application/x-www-form-urlencoded")
                            .build();
                    response = client.newCall(request).execute();
                    mediaType = response.body().contentType();
                    doc = Jsoup.parse(response.body().byteStream(), mediaType.charset().name(),
                            "https://recifeemdia.recife.pe.gov.br/");

                    lblMensagem = doc.selectFirst("label[id=lblMensagem]");
                    if(Objects.nonNull(lblMensagem)
                            && lblMensagem.text().indexOf("código de verificação") != -1) {
                        //capmonster.report(imageToTextTask.getTaskId(), false);
                        throw new Exception("O código de verificação informado está incorreto.");
                    }

                    if(Objects.nonNull(lblMensagem)
                            && lblMensagem.text().indexOf("invalid csrf") != -1) {
                        //capmonster.report(imageToTextTask.getTaskId(), false);
                        throw new Exception(lblMensagem.text());
                    }

                    if(Objects.nonNull(lblMensagem) && lblMensagem.text().length() > 0) {
                        SQSUtil.status(document.get("id").toString(), key, "FAIL", lblMensagem.text(), message.getId());
                        //capmonster.report(imageToTextTask.getTaskId(), true);
                        continue;
                    }

                    //capmonster.report(imageToTextTask.getTaskId(), true);

                    loginForm = (FormElement) doc.selectFirst("form[id=login-form]");
                    if(Objects.nonNull(loginForm)) {
                        throw new Exception("Tente novamente.");
                    }

                    fixDoc(doc, "https://recifeemdia.recife.pe.gov.br");

                    InputStream inputStream = IOUtils.toInputStream(doc.html(), mediaType.charset());
                    InputStream inputStreamPDF = PdfService.htmlToPdf(inputStream);
                    byte[] bytes = PdfService.signature(inputStreamPDF, keyEntry);
                    StorageUtil.upload(key, bytes, "application/pdf");

                    boolean hasInvoice = false;

                    Element formExtratoDebitos = doc.selectFirst("form[id=formExtratoDebitos]");
                    if(Objects.nonNull(formExtratoDebitos)) {
                        Elements tabelaAdministrativos = formExtratoDebitos.select("div[class='table-responsive tabelaAdministrativo']");
                        for (Element parcelamento : tabelaAdministrativos) {
                            for (Element tbody : parcelamento.select("tbody")) {
                                for (Element row : tbody.select("tr")) {
                                    Elements tds = row.select("td");
                                    for (Element link : tds.select("a")) {
                                        if (link.hasAttr("data")) {

                                            hasInvoice = true;

                                            try {
                                                String json = link.attr("data");
                                                Map<String, Object> jsonData = new Gson().fromJson(json, Map.class);
                                                String tipoDebito = (String) jsonData.get("tipoDebito");

                                                if (tipoDebito.equalsIgnoreCase("ADMINISTRATIVO")) {

                                                    List<Map<String, Object>> detalhamentos = (List<Map<String, Object>>) jsonData.get("detalhamentos");
                                                    String parcelas = detalhamentos.stream().map(detalhamento -> detalhamento.get("parcelas").toString())
                                                            .collect(Collectors.joining(","));

                                                    FormBody.Builder formBody = new FormBody.Builder();
                                                    formBody.add("tipoDebito", tipoDebito);
                                                    formBody.add("processo", (String) jsonData.get("processo"));
                                                    formBody.add("certidao", "");
                                                    formBody.add("inscricao", document.get("municipalCode").toString().replaceAll("[^0-9]", ""));
                                                    formBody.add("tipo", String.valueOf((int) ((double) jsonData.get("tipoProcesso"))));
                                                    formBody.add("ano", (String) jsonData.get("exercicio"));
                                                    formBody.add("parcelas", parcelas);
                                                    //formBody.add("dataVencimento", LocalDate.now().plusDays(10).format(DateTimeFormatter.ofPattern("ddMMyyyy")));

                                                    request = new Request.Builder()
                                                            .url("https://recifeemdia.recife.pe.gov.br/servicos/validarDam")
                                                            .method("POST", formBody.build())
                                                            .addHeader("Content-Type", "application/x-www-form-urlencoded")
                                                            .build();
                                                    response = client.newCall(request).execute();
                                                    String responseBody = HTTPUtil.bodyToString(response, mediaType.charset(Charset.forName("UTF-8")));

                                                    Gson gson = new GsonBuilder()
                                                            .setObjectToNumberStrategy(ToNumberPolicy.LONG_OR_DOUBLE)
                                                            .create();

                                                    Map<String, Object> hashMap = gson.fromJson(responseBody, Map.class);
                                                    if (hashMap.containsKey("status")) {
                                                        String status = (String) hashMap.get("status");
                                                        if (status.equalsIgnoreCase("error")) {
                                                            String error = (String) hashMap.get("data");
                                                            throw new Exception(error);
                                                        }
                                                    }

                                                    request = new Request.Builder()
                                                            .url("https://recifeemdia.recife.pe.gov.br/gerarDam/" + hashMap.get("data").toString() + "/")
                                                            .method("GET", null)
                                                            .build();
                                                    response = client.newCall(request).execute();
                                                    mediaType = response.body().contentType();

                                                    String invoiceKey = data.get("workspaceCode").toString() + "/invoice/" +
                                                            document.get("companyCode").toString().replaceAll("[^0-9]", "") + "/" +
                                                            "municipal/" +
                                                            document.get("municipalCode").toString().replaceAll("[^0-9]", "") + "/" +
                                                            LocalDate.now().format(DateTimeFormatter.ofPattern("yyMM")) + "/relatorio-dam-" +
                                                            UUID.randomUUID() + ".pdf";

                                                    bytes = PdfService.signature(response.body().byteStream(), keyEntry);
                                                    StorageUtil.upload(invoiceKey, bytes, "application/pdf");

                                                    HashMap<String, Object> dataMessage = new HashMap<>();
                                                    dataMessage.put("key", jsonData.get("processo") + "/" + parcelas);
                                                    dataMessage.put("uf", message.getUf().name());
                                                    dataMessage.put("workspaceCode", data.get("workspaceCode"));
                                                    dataMessage.put("companyCode", document.get("companyCode").toString().replaceAll("[^0-9]", ""));
                                                    dataMessage.put("municipalCode", document.get("municipalCode").toString().replaceAll("[^0-9]", ""));
                                                    dataMessage.put("storageKey", invoiceKey);
                                                    dataMessage.put("type", "DAM");
                                                    dataMessage.put("message", tipoDebito);
                                                    LocalDate dueDate = jsonData.containsKey("vencimento") ?
                                                            LocalDate.parse(((String) jsonData.get("vencimento")), DateTimeFormatter.ofPattern("dd/MM/yyyy")) :
                                                            LocalDate.now();
                                                    dataMessage.put("dueDate", dueDate.format(DateTimeFormatter.ISO_LOCAL_DATE));
                                                    dataMessage.put("price", jsonData.get("totalAVista").toString()
                                                            .replaceAll("[^0-9\\.\\,]", "")
                                                            .replaceAll("\\.", "")
                                                            .replaceAll("\\,", "\\."));

                                                    HashMap<String, Object> dataHeader = new HashMap<>();
                                                    dataHeader.put("id", UUID.randomUUID().toString());
                                                    dataHeader.put("action", "Invoice");
                                                    dataHeader.put("date", LocalDateTime.now().format(DateTimeFormatter.ISO_DATE_TIME));
                                                    dataHeader.put("data", dataMessage);

                                                    SQSUtil.sendMessage(dataHeader, message.getId());
                                                }
                                            } catch (Exception exception) {
                                                exception.printStackTrace();
                                            }
                                        }
                                    }
                                }
                            }
                        }
                    }

                    SQSUtil.status(document.get("id").toString(), key, (hasInvoice ? "FOUND_ISSUE" : "DONE"), null, message.getId());

                } catch (CaptchaException | Exception exception) {
                    errors.add(document);
                    exception.printStackTrace();
                } finally {
                    stopWatch.stop();
                    long seconds = (stopWatch.getTotalTimeMillis() / 1000);
                    if(seconds >= visibilityTimeout) {
                        throw new ExpiredException("Message expired: " + message);
                    }
                    SQSUtil.changeMessageVisibility(queueUrl, receiptHandle, visibilityTimeout);
                }
            }
            SQSUtil.resend(queueUrl, message, "documents", errors, messageGroupId);

            acknowledgment.acknowledge();
        } catch (CertificateException exception) {
            exception.printStackTrace();
            acknowledgment.acknowledge();
        } catch (Exception | ExpiredException e) {
            throw new Exception(e);
        }
    }
}
